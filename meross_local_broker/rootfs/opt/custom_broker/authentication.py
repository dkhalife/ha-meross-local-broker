...
def _user_logout(token: str) -> None:
    ...
    dbhelper.store_event(EventType.USER_LOGOUT, details=f"User {user.email} has failed to log in via HTTP API: wrong or unexisting email specified.", user_id=user.user_id);
...
def _attempt_password_upgrade(email, password) -> bool:
    ...
    if user is None:
        dbhelper.store_event(EventType.USER_LOGIN_FAILURE, details=f"User {email} has failed to log in via HTTP API: wrong or unexisting email specified.");
        return False
...
def _user_login(email: str, password: str, providing_pre_hashed_password: bool) -> Tuple[User, UserToken]:
    ...
    if computed_hashed_password != user.password:
        dbhelper.store_event(EventType.USER_LOGIN_FAILURE, details=f"User {email} has failed to log in via HTTP API: wrong password.");
    ...
    dbhelper.store_event(EventType.USER_LOGIN_SUCCESS, details=f"User {email} has successfully logged in via HTTP API.");
    return user, token
